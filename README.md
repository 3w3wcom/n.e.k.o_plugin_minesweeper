...
```text
N.E.K.O/plugin/plugins/minesweeper
```
...
```bash
uv run --with pip neko-plugin sync minesweeper --clean
uv run neko-plugin check minesweeper
uv run neko-plugin check -r minesweeper
```
...
```bash
uv run neko-plugin publish minesweeper
```

To run only one half explicitly / 如需仅执行一部分 / 一方のみを実行する場合:

```bash
uv run neko-plugin publish github minesweeper
...
`minesweeper.neko-plugin`. The market independently verifies that Release
...
```toml
entry = "plugin.plugins.minesweeper:MinesweeperPlugin"
```
